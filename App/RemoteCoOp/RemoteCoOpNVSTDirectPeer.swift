...
public struct RemoteCoOpNVSTDirectPeerFactory: RemoteCoOpHostPeerFactory {
    public init() {}
    ...
    public func makePeer(participantID: UUID,
        ...
        RemoteCoOpNVSTDirectPeer(participantID: participantID,
...
public final class RemoteCoOpNVSTDirectPeer: NSObject, RemoteCoOpHostPeer, RemoteCoOpHostVideoSink, RemoteCoOpHostAudioSink, RTCPeerConnectionDelegate, RTCDataChannelDelegate, @unchecked Sendable {
    public let participantID: UUID
    
    private static let inputChannelLabel = "input_channel_partially_reliable"
    ...
    public init(participantID: UUID,
        ...
        self.videoQueue = DispatchQueue(label: "io.github.opencloudgaming.pixelnow.remote-coop.nvst-video.\(participantID.uuidString)")
        super.init()
    }
    ...
    private func makePeerConnection() throws -> RTCPeerConnection {
        ...
        let factory = RTCPeerConnectionFactory(encoderFactory: encoderFactory, decoderFactory: decoderFactory, audioDevice: nil)
        ...
        WebRTCMediaTelemetry.capture("webrtc.remote_coop.nvst_direct.connection", level: .info, message: "NVST direct P2P peer connection created.", attributes: ["participantID": participantID.uuidString, "iceServers": String(configuration.iceServers.count), "policy": networkConfiguration.iceTransportPolicy.rawValue])
    ...
    private func createInputChannel(peerConnection: RTCPeerConnection) {
        ...
        stateLock.withLock { inputChannels.append(channel) }
    }
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<RTCSessionDescription, Error>) in
            peerConnection.offer(for: constraints) { offer, error in
                if let offer {
                    ...
                    continuation.resume(throwing: RemoteCoOpHostPeerError.negotiationFailed(error?.localizedDescription ?? "Unable to create NVST direct P2P WebRTC offer."))
                }
            }
        }
    }
